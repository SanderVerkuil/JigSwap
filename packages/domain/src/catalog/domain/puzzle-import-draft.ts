...
// `images` is the full ordered, deduplicated list (JSON-LD first, then OG); `imageUrl` is
// always `images[0]` — kept for the single-image importPuzzleImage path.
...
export interface RawProductPage {
  ...
  // Set by each fetcher adapter so the use case can log which tier served the page.
