...
export default function PuzzlesPage() {
  ...
  const userPuzzles = useQuery(api.puzzles.getPuzzlesByOwner,
    convexUser?._id ? { ownerId: convexUser._id, includeUnavailable: true } : "skip"
  );
  ...
  const handleDeletePuzzle = async (puzzleId: string) => {
    if (confirm("Are you sure you want to delete this puzzle?")) {
      try {
        await deletePuzzle({ puzzleId: puzzleId as any });
      } catch (error) {
  ...
  const filteredPuzzles = userPuzzles?.filter(puzzle =>
      ...
      {filteredPuzzles.length === 0 ? (
        ...
        <div className={viewMode === "grid" 
          ...
          {filteredPuzzles.map((puzzle) => (
            <Card key={puzzle._id} className={viewMode === "list" ? "flex" : ""}>
              {viewMode === "grid" ? (
                <>
                  <div className="aspect-square bg-muted rounded-t-lg relative overflow-hidden">
                    ...
                    <div className="absolute top-2 right-2">
                      <Badge variant={puzzle.isAvailable ? "default" : "secondary"}>
