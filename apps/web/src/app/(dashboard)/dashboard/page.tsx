    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{userStats?.puzzlesOwned || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{userStats?.tradesCompleted || 0}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {recentTrades?.filter(t => t.status === "pending" || t.status === "accepted").length || 0}
            </div>
            <p className="text-xs text-muted-foreground">
            ...
            {recentPuzzles && recentPuzzles.length > 0 ? (
              <div className="space-y-3">
                {recentPuzzles.slice(0, 3).map((puzzle) => (
                  <div key={puzzle._id} className="flex items-center space-x-3 p-3 border rounded-lg">
            ...
            {recentTrades && recentTrades.length > 0 ? (
              <div className="space-y-3">
                {recentTrades.slice(0, 3).map((trade) => (
                  <div key={trade._id} className="flex items-center space-x-3 p-3 border rounded-lg">
                    <div className="w-12 h-12 bg-muted rounded-lg flex items-center justify-center">
                      <ArrowLeftRight className="h-6 w-6" />
                    </div>
                    <div className="flex-1">
                      <p className="font-medium">
                        {trade.userRole === "requester" ? "Requested" : "Received request for"}: {trade.ownerPuzzle?.title}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Status: {trade.status}
                      </p>
